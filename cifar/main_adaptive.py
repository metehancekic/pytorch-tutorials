...
def main():
    ...
    if args.frontend == "Identity":
        ...
        from ..models.custom_layers import CenterSurroundModule, AutoEncoder_adaptive, Decoder, CenterSurroundConv, DoGLayer, DoGLowpassLayer, LowpassLayer, DoG_LP_Layer, LP_Gabor_Layer, LP_Gabor_Layer_v2, LP_Gabor_Layer_v3, LP_Gabor_Layer_v4,  LP_Gabor_Layer_v5,  LP_Gabor_Layer_v6, LP_Layer, Identity
        frontend = locals()[args.frontend](beta=args.beta, BPDA_type=args.bpda_type).to(device)
        CNN = locals()[args.model]().to(device)
        model = AutoEncoder_adaptive(frontend, CNN).to(device)
